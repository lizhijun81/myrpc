...
public class Exporter implements Serializable {
	...
	public Exporter(URL url) {
		...
		this.registry = ExtensionLoader.getExtensionLoader(Registry.class)
				.getExtension(url.getParameter(RpcConstants.REGISTRY_NAME_KEY), Scope.SINGLETON).with(url).init();
	}
