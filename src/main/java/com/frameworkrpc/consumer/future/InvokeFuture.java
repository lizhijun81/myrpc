...
public class InvokeFuture<V> implements ListenableFuture<V> {

	private static final Logger logger = LoggerFactory.getLogger(InvokeFuture.class);
	public static final ConcurrentHashMap<String, InvokeFuture> pendingRPC = new ConcurrentHashMap<>();
	...
	public InvokeFuture(RpcRequest request) {
		...
		try {
			...
			if (this.response != null) {
				if (this.response.getError() != null) {
					...
					throw new MyRpcInvokeException(this.response.getError());
				}
			} else {
				return null;
			}
	...
	private void await(long timeout, TimeUnit unit) {
		...
		if (!isTimeout) {
			...
			throw new MyRpcInvokeException(
	...
	@Override
	public ListenableFuture<V> addListeners(FutureListener<V>... listeners) {
		return null;
	}
	...
	@Override
	public ListenableFuture<V> removeListeners(FutureListener<V>... listeners) {
		return null;
	}
}
